 {!! Form::open(['url' => url('club/user/update/' . $user->id), 'method' => 'POST']) !!}
              ...
              <div class="row">
                ...
                @foreach ($club->clubPlaygrounds as $playground)
                  <div class="col-lg-4">
                    <div class="checkbox">
                        <label><input type="checkbox" name="playgrounds[]" value="{{ $playground->id }}"
                        @foreach ($user->playgrounds as $user_playgrounds)
                          @if ($user_playgrounds->id == $playground->id)
                            checked="checked"
                          @endif
                        @endforeach
                      >{{ $playground->c_b_p_name }}</label>
                    </div>
                  </div>
                @endforeach
              </div>
